...
veryOldInventors = inventors.filter((num) => {
  return num.year >= "1500" && num.year < "1600";
});
...
let inventorNames = [];
inventorNames = inventors.map((str) => {
  return { first: str.first, second: str.last };
});
...
sortedByBirthYear = inventors.sort((num1, num2) => {
...
inventorNamedAda = inventors.find((str) => {
  return str.first === "Ada";
});
...
// Enter your solution code here:
isAdultPresent = devs.some((num) => {
  return 2024 - num.year >= 18;
});
...
// Enter your solution code here:
isEveryone19OrOlder = devs.every((num) => {
  return 2024 - num.year >= 19;
});
...
// Enter your solution code here:
totalYearsLived = inventors.reduce((accumulator, total) => {
  return accumulator + (total.passed - total.year);
}, 0);
